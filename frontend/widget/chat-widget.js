(function () {
  ...
  async function startChat() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v1/chat/init`, {
        method: 'POST',
  ...
  function addMessage(msg) {
    ...
    div.innerHTML = `
      <div class="hd-bubble">${msg.content.replace(/\n/g, '<br>')}</div>
  ...
  async function sendMessage() {
    ...
    await fetch(`${API_URL}/api/v1/chat/sessions/${sessionId}/visitor-message`, {
      method: 'POST',
  ...
  function startPolling() {
    pollInterval = setInterval(async () => {
      if (!sessionId) return;
      try {
        const url = `${API_URL}/api/v1/chat/sessions/${sessionId}/poll${lastTs ? '?since=' + encodeURIComponent(lastTs) : ''}`;
